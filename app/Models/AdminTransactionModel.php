    ...
    public function getChinaTransaction($request) {
        ...
        $data = DB::table('admin_transactions')
            ...
            ->select(
                ...
                'admin_transactions.create_at',
                'admin_transactions.surplus'
            )
            ->whereNotNull('admin_transactions.point_cn')
            ->orderByDesc('admin_transactions.create_at');
        ...
        if ($create_to) {
            $data->whereDate('admin_transactions.create_at', '>=', $create_to);
        }
        if ($create_from) {
            $data->whereDate('admin_transactions.create_at', '<=', $create_from);
        }
